...
T3 cross-scale 0.865, and **confirmed CROSS-ARCHITECTURE on three held-out families —
SmolLM2-1.7B (near/Llama 0.810), Gemma-2-2b-it (far 0.844), OLMo-2-1B (far 0.838), all
T5 PASS ≥ 0.70** — needs no activations, so it is the shippable model-agnostic gate. **All
...
`pathway11_h100/generalization_edge/RESULTS_v6_phase234.md`. Far-family T5 CLOSED
(Gemma + OLMo-2 both PASS, EXP-82 follow-on on a RunPod H100 — user-authorized for
speed, deterministic greedy so hardware-independent, JSON re-evaluated locally). Open:
cross-scale arm-2A (7B prompt clouds GPU-blocked locally — moot, cloud loses in-domain).
...
`validate_claims.py` invariant: **223/223 internal PASS, 267 claims tracked total**
(223 internal + 41 external + 3 PENDING_FE) — +9 internal in EXP-82 (edge-v6: SmolLM2
T5 free/length, **far-family Gemma free/logprob + OLMo-2 free**, arm-2A prompt-length +
gate-null, Phase-4 cross-scale ceiling + conformal validity). Prior +6 (FE19: PANL
best/L19 AUROC, verbalized AUROC, verify-then-correct).
...
greedy on **three** held-out families (zero role in selection); free length+logprob
OOF AUROC clears ≥0.70 on all — **SmolLM2-1.7B** (near/Llama, 0.8097), **Gemma-2-2b-it**
(far, 0.8439), **OLMo-2-1B** (far, 0.8382) — so the shippable, zero-activation gate
generalizes cross-architecture. *Which* scalar carries is family-dependent (SmolLM2
length-only, logprob→chance 0.518; Gemma both 0.783/0.756; OLMo-2 length-led
0.816/0.654) — which is why pinning *both* makes the gate robust. SmolLM2 ran locally;
Gemma + OLMo-2 ran on a RunPod H100 (EXP-82 follow-on, user-authorized for speed;
deterministic greedy so hardware-independent, canonical JSON re-evaluated locally from
all three `.npz`). `results/phase3_t5_free_baseline.json`.
...
cross-architecture to three unseen families (T5 SmolLM2 0.81 / Gemma 0.84 / OLMo-2 0.84);
geometry fully closed; new Tier-A prompt-length signal (0.71); guarantees transfer
zero-shot to a stronger model but not to a harder domain. Far-family T5 closed
(Gemma+OLMo-2, H100 follow-on); open: cross-scale arm-2A (GPU-blocked, moot). H-E
refuted (added).
