# Generalization-First Edge Program — SPEC v5
...
- **H-F (selection, 1B.1):** probe-reranking beats K=8 plain majority (55.0%) at identical compute
  ...
  comparative* signal that unweighted voting discards.
...
- **H-H (guarantees, 1B.3):** split-conformal thresholds on OOF scores achieve nominal coverage on
  ...
  linking guarantee validity to the program's central metric.
...
5B. **Phase-1B anchors (V4-1/V4-2, must reproduce from caches before trusting any 1B number):**
   K=1 greedy 0.486, K=8 plain majority 0.550, all-7B 0.732, FU2 hybrid AUROC 0.852 and
   savings-gap 11.2pp at the 95% target, G1 pre-gen point (keep 5% → 0.724 @ 0.96×). Plus the
   1B.1 position check: `L19_samples` confirmed final-token (or probe refit at the true position).
...
- File a new umbrella `:FutureExperiment` for the program (and mark the constituent nodes —
  FE238/FE55/FE357/FE339/FE301/FE268/FE254/FE70, **plus FE41 when 1B.1 runs (executes H-40)** —
  COMPLETED/SUBSUMED as each is run) via `research-graph/update_status.py`; regenerate
  `NEXT_EXPERIMENTS.md`.
- **Triage the conformal-adjacent graph entries (2205.14334, 2402.10978, 2406.15927) before 1B.3
  makes any novelty claim (V4-4).**
...
- **Step 0 of execution:** write this spec into the repo at
  `pathway11_h100/generalization_edge/SPEC_v5.md` so it is durable beyond the plan file.
