#!/usr/bin/env python3
"""Phase 6.5 Step 2: MATH-500 × 7B with max_new_tokens=1024.

GPU REQUIRED. Run on RunPod H100.

Fixes the truncation confound for Qwen2.5-7B-Instruct on MATH-500:
- max_new_tokens increased from 256 to 1024
- Same 400/100 train/holdout split as 1.5B (seed 9999 mask)
- Reuses Phase 3 greedy trajectories when available

Expected outcome: accuracy ~75-77% (vs 16.2% with truncation), deconfounded AUROC.
...
PHASE3_MATH7B_DIR = Path(__file__).parent.parent / "phase3_cross_benchmark" / "math7b"
...
def run_greedy(model, tokenizer, prompts, ground_truths):
    """Generate greedy answers with max_new_tokens=1024 for all 500 problems."""
    ...
    logger.info("Model: %s", model.config.name_or_path)
    logger.info("Layers: %d, Hidden dim: %d", model.config.num_hidden_layers, model.config.hidden_size)

    # Compare with Phase 3
    phase3_path = PHASE3_MATH7B_DIR / "baseline_correct.npy"
...
def load_or_extract_trajectories(model, tokenizer, prompts):
    ...
    traj_path = PHASE3_MATH7B_DIR / "trajectories.npz"
    ls_path = PHASE3_MATH7B_DIR / "layer_states.npy"
...
def run_features_and_model(trajectories, layer_states, correct, entropy_answers):
    ...
    Uses the SAME 400/100 split as 1.5B (seed 9999 mask). Unlike GSM8K,
    the split is deterministic from the mask, not from labels.
    """
    n = len(correct)
    ...
    logger.info(
        "  Train: %d correct (%.1f%%), Holdout: %d correct (%.1f%%)",
        correct[train_idx].sum(), 100 * correct[train_idx].mean(),
        correct[holdout_idx].sum(), 100 * correct[holdout_idx].mean(),
    )
    ...
    # Compare with Phase 3
    phase3_results = PHASE3_MATH7B_DIR / "model_results.json"
...
def main():
    t_start = time.time()
    logger.info("=" * 70)
    logger.info("PHASE 6.5 STEP 2: MATH-500 × 7B (max_new_tokens=%d)", MAX_NEW_TOKENS_65)
    logger.info("=" * 70)
    ...
    # Section 1: Greedy (max_new_tokens=1024)
    ...
    if acc < 0.60:
        logger.error(
            ...
            "Investigate prompt template and answer extraction.",
            100 * acc,
        )

    # Section 2: Trajectories (reuse Phase 3 if available)
    ...
    logger.info("\n" + "=" * 70)
    logger.info("MATH-500 × 7B PIPELINE COMPLETE (max_new_tokens=%d)", MAX_NEW_TOKENS_65)
    logger.info("=" * 70)
