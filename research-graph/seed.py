...
PAPER_EDGES: list[tuple[str, str, str, dict[str, Any]]] = [
    ...
    ("F-2", "CORROBORATED_BY", "2412.15296",
     {"note": "Confidence in reasoning — hidden-state confidence signals predict correctness, aligns with prefill-knows-best"}),
    ...
    ("F-2", "EXTENDED_BY", "2504.05419",
     {"experiment_idea": "Mid-trajectory probes at chunk boundaries get >0.9 AUROC on AIME — we should test chunk-boundary probing",
      "actionable": True}),
    ("F-2", "EXTENDED_BY", "2509.24988",
     {"note": "Generalized correctness models — model-agnostic calibrated predictors from historical patterns"}),
    ...
    ("F-2", "METHOD_DIFFERS", "2602.13567",
     {"theirs": "DistillLens symmetric knowledge distillation through logit lens",
      "ours": "Prefill DoM direction at L19"}),

    # F-3 direction rotation
    ("F-3", "CORROBORATED_BY", "2406.11717",
     {"note": "Refusal mediated by single direction — confirms directional control exists but is context-dependent"}),
    ("F-3", "CORROBORATED_BY", "2506.18167",
     {"note": "Steering vectors in thinking LMs — confirms direction rotation matters for reasoning models"}),
    ...
    ("F-3", "EXPLAINS", "2310.13121",
     {"mechanism": "Understanding addition in transformers — mechanistic basis for how directions encode computation"}),
    ("F-3", "EXPLAINS", "2407.12404",
     {"mechanism": "Steerability is context-dependent; cached vectors are unreliable across distributions"}),
    ("F-3", "EXPLAINS", "2602.02343",
     {"mechanism": "Why steering works — unified view of parameter dynamics explains direction rotation"}),
    ("F-3", "EXTENDED_BY", "2406.00045",
     {"note": "Personalized steering via bi-directional preference optimization — handles per-user direction variation"}),
    ("F-3", "EXTENDED_BY", "2410.12877",
     {"note": "Improving instruction-following through activation steering — extends fixed-vector approach"}),
    ("F-3", "EXTENDED_BY", "2501.09929",
     {"note": "Feature-guided activation additions — interpretable steering may handle rotation better"}),
    ...
    ("F-3", "EXTENDED_BY", "2510.04309",
     {"experiment_idea": "PID steering accumulates error across layers to handle rotation — directly addresses our failure mode",
      "actionable": True}),
    ("F-3", "EXTENDED_BY", "2601.13015",
     {"note": "Multi-expert LLMs with inference-time intervention for RTL code generation"}),
    ("F-3", "EXTENDED_BY", "2601.19375",
     {"note": "Selective steering with norm-preserving discriminative layer selection — addresses rotation by choosing when to steer"}),
    ("F-3", "EXTENDED_BY", "2602.04428",
     {"note": "Fine-grained activation steering — steer less, achieve more; partial steering may sidestep rotation"}),
    ("F-3", "EXTENDED_BY", "2602.07276",
     {"note": "Steer2Adapt — dynamically composing steering vectors for efficient adaptation"}),
    ("F-3", "EXTENDED_BY", "2602.11910",
     {"note": "TADA — activation steering for audio diffusion; cross-domain validation of steering approach"}),
    ("F-3", "METHOD_DIFFERS", "2410.04962",
     {"theirs": "Activation scaling for steering and interpreting LMs",
      "ours": "DoM direction rotation measurement showing fixed vectors fail"}),
    ...
    ("F-4", "EXTENDED_BY", "2404.12038",
     {"note": "Concept activation vectors for safety risk detection — extends DoM-style probing to safety domain"}),
    ("F-4", "EXTENDED_BY", "2506.18167",
     {"note": "Steering vectors in thinking LMs — extends linear representation to reasoning models"}),
    ("F-4", "METHOD_DIFFERS", "2310.17230",
     {"theirs": "Codebook features — sparse discrete interpretability",
      "ours": "Raw DoM continuous direction"}),
    ("F-4", "METHOD_DIFFERS", "2501.09929",
     {"theirs": "Feature-guided activation additions with SAE features",
      "ours": "Raw DoM direction without dictionary learning"}),
    ("F-4", "METHOD_DIFFERS", "2502.04043",
     {"theirs": "Probe-free low-rank activation intervention",
      "ours": "Single DoM probe at L19"}),
    ("F-4", "METHOD_DIFFERS", "2509.24988",
     {"theirs": "Generalized correctness models from historical patterns",
      "ours": "Single-pass DoM probe from hidden states"}),
    ("F-4", "METHOD_DIFFERS", "2602.00158",
     {"theirs": "RAPTOR ridge-adaptive logistic probes",
      "ours": "Raw DoM direction at L19"}),

    # F-5 dimensional breathing scales with model size
    ("F-5", "CORROBORATED_BY", "2602.13567",
     {"note": "DistillLens — logit lens reveals cross-scale representation patterns consistent with breathing-scales-with-size"}),
    ...
    ("F-6", "METHOD_DIFFERS", "2507.16806",
     {"theirs": "Beyond binary rewards — training LMs to reason about uncertainty",
      "ours": "CoE-60 geometric trajectory features from single pass"}),
    ...
    ("F-11", "CORROBORATED_BY", "2412.15296",
     {"note": "Confidence in reasoning — hidden-state confidence signals support selective prediction approach"}),
    ("F-11", "EXTENDED_BY", "2402.10978",
     {"experiment_idea": "Wrap our DoM probe in conformal prediction for distribution-free coverage guarantees",
      "actionable": True}),
    ("F-11", "EXTENDED_BY", "2507.16806",
     {"note": "Beyond binary rewards — LMs trained to reason about uncertainty; extends selective prediction paradigm"}),
    ("F-11", "EXTENDED_BY", "2604.20614",
     {"note": "CalMO — calibration for math-oriented reasoning; directly applicable to our selective prediction stack"}),
    ("F-11", "METHOD_DIFFERS", "2205.14334",
     {"theirs": "Teaching models to express uncertainty in words (verbalized confidence)",
      "ours": "Hidden-state DoM-based selective prediction"}),
    ("F-11", "METHOD_DIFFERS", "2406.15927",
     {"theirs": "SEP-based abstention",
      "ours": "Prefill DoM-based refusal"}),
    ("F-11", "METHOD_DIFFERS", "2505.19184",
     {"theirs": "Debate-based confidence — two LLMs debate, both think they win",
      "ours": "Single-pass hidden-state DoM probe"}),
