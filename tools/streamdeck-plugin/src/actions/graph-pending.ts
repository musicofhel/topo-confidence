...
@action({ UUID: "com.topoconfidence.daemon-monitor.graph-pending" })
export class GraphPending extends SingletonAction<MonitorSettings> {
	...
	override async onWillAppear(ev: WillAppearEvent<MonitorSettings>): Promise<void> {
		...
		const btn: ButtonState = { animTimer: null, flashOn: true, pending: 0, action: ev.action };
	...
	override async onDidReceiveSettings(ev: DidReceiveSettingsEvent<MonitorSettings>): Promise<void> {
		...
		if (!btn) {
			btn = { animTimer: null, flashOn: true, pending: 0, action: ev.action };
			this.buttons.set(id, btn);
		}
		pipelinePoller.unregister(id);
		pipelinePoller.register(id, (data) => this.onUpdate(id, data), ev.payload.settings.serverUrl || "http://localhost:9876");
	...
	private onUpdate(id: string, data: PipelineResponse | null): void {
		...
		const wasPending = btn.pending > 0;
		btn.pending = data.research_graph_pending;

		if (btn.pending > 0 && !wasPending) {
			this.startFlash(btn);
		} else if (btn.pending === 0 && wasPending) {
			this.stopFlash(btn);
			this.render(btn);
		} else if (btn.pending === 0) {
			this.render(btn);
		}
	}
	...
	private render(btn: ButtonState): void {
		...
		if (btn.pending < 0) {
			...
			color = "yellow";
			subtitle = String(btn.pending);
		} else {
			color = "green";
			subtitle = "0";
		}

		const svg = renderMonitorSvg({ title: "GRAPH", subtitle, color, flashOn: btn.flashOn });
